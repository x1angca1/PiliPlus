...
class PlayerFocus extends StatelessWidget {
  ...
  bool _handleKey(BuildContext context, KeyEvent event) {
    ...
    if (event is KeyDownEvent && !plPlayerController.isLive) {
      if (key == Pref.frameBackwardKey) {
        plPlayerController.frameStep(backward: true);
        return true;
      }
      if (key == Pref.frameForwardKey) {
        plPlayerController.frameStep();
        return true;
      }
    }
